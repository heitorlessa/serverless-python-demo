...
from tests.integration.stream_processor.conftest import get_event_from_table


def test_eventbridge_provider_send(request: FixtureRequest):
    ...
    event_bus_name = 'lessa-stream-processor-ProductStreamProcevents'  # todo: fetch from stack output
    ...
    time.sleep(1)
    event_received = get_event_from_table(table_name='test-eb-events', event_source=event_source, event_name=event_name, receipt_id=receipt_id)
